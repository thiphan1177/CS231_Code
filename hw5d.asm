...
_start:
	...
	;; to number of characters the string has. 
		...
		mov	esi, ecx      ; move string address (in ecx) to esi
		mov	ecx, edx      ; move string's # of chars (in edx) to ecx
		mov	edx, 1
		dec	ecx	      ; decrement ecx by 1 so that _printDec prints the right decimal
		mov	al, byte[esi] ; move 1st letter into reg al

;;; Do checksum!!
loopthis:
		mov	bl, byte[esi+edx] ; move subsequent letter into reg bl
		xor	al, bl
		inc	edx
		loop	loopthis
